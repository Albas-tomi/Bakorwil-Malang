  ...
  return (
    <>
      {Data.slice(0, 2).map((dataPengumuman) => (
        <div
          className="card md:grid md:grid-cols-1 lg:grid-cols-5 gap-3 items-stretch object-cover cursor-pointer"
          onClick={() => {
        ...
        >
          <div className="card card-compact col-span-2 bg-base-100 bg-cover rounded-md overflow-hidden shadow-xl md:shadow-none">
            <img src={dataPengumuman.image} alt={dataPengumuman.title} className="h-28 md:h-48 lg:h-16 object-cover" />
          ...
          <h3 className="hidden md:inline-block col-span-3 md:text-xs lg:text-sm font-medium capitalize place-self-center">{dataPengumuman.title}</h3>
        </div>
      ))}
    </>
  );
