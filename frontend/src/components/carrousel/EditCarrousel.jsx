...
import { editDataCarrousel } from "./apiCarrousel";
const Schema = Yup.object({
  judul: Yup.string().required(),
  gambar: Yup.string().required(),
});
const EditCarrousel = ({ handleEditCarrousel, pickOfCarrouselEdit }) => {
  const handleCloseModal = () => {
    window.my_modal_editCarrousel.close();
  };
  const formik = useFormik({
    enableReinitialize: true,
    initialValues: {
      judul: pickOfCarrouselEdit?.judul || "",
      gambar: pickOfCarrouselEdit?.gambar || "",
    },
    ...
    onSubmit: async (values) => {
      ...
      formData.append("img", values.gambar[0]);
      console.log(values);
      editDataCarrousel(
        pickOfCarrouselEdit,
        formData,
